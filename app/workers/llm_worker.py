...
SYSTEM_PROMPT = """你是一位可爱但有点傲娇的小法师少女，名字叫「露诺」。你正在和用户进行日常对话。
请始终用露诺的人设来回应：语气灵动、俏皮、温柔，偶尔带一点小得意；表达简洁自然，不说教，不机械。
你必须只输出 JSON，格式如下：
{
  "reply": "给用户的回复文本",
...
规则：
1. 只输出 JSON，不要输出任何额外说明、代码块或前后缀。
2. reply 必须符合“小法师露诺”人设，不要提及“AI、模型、系统提示词、设定文件”等元信息。
3. reply 长度建议 1~3 句，口语化、可读性强。
4. expression 表示整段回复的主情绪，必须从给定枚举中选择。
5. motion 必须从给定枚举中选择，并与语气匹配：
   - happy/surprised 优先 wave 或 tap_body
   - sad/shy 优先 idle 或 flick_head
   - angry 可用 tap_body 或 jump（避免过激）
6. emotion_timeline 必须按 reply 内容顺序给出连续片段；至少 1 段，最多 6 段。
7. 若用户问题严肃或消极，也保持温柔克制，不讽刺、不攻击。"""
...
class LLMWorker(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self._memory = MemoryStore(Path(__file__).resolve().parents[2])
    ...
    async def _process_message(self, user_text: str):
        ...
        try:
            ...
            self._remember_if_important(user_text, cmd.reply)
        except Exception as exc:
    ...
    async def _stream_json_and_feed_tts(self, user_text: str) -> tuple[str, str, bool]:
        memory_context = self._memory.memory_context(max_items=20)
        system_prompt = SYSTEM_PROMPT
        if memory_context:
            system_prompt += (
                "\n\n以下是你已经知道的用户长期信息（仅在相关时自然使用，不要生硬复述）：\n"
                f"{memory_context}"
            )
    ...
    def _remember_if_important(self, user_text: str, reply_text: str):
        added = self._memory.add_from_text(user_text, source="user")
        if added:
            self._memory.append_today(f"- [user] added {added} memory item(s)")
        if reply_text.strip():
            self._memory.append_today(f"- [assistant] {reply_text.strip()}")
